 ...
 * под 0.5% стоп.
 *
 * Имаше един - "Спринтьор", стоп 0.3%, цел 1:2. Махнат е, след като три
 * независими проверки казаха едно и също:
 *
 *   - върху история, вход във всеки момент: 20.7% при нужни 53.3%
 *   - със същите порти върху история: 22.0% - портите не го поправиха
 *   - живо проследяване, 6 часа: 33.3%, нето -1.40%
 *
 * И дори с НУЛЕВИ такси нулата му остава 33.3%, докато Пазителят иска 27.3%
 * при пълни такси. Проблемът не е в таксите, а в геометрията: стоп 0.3% с
 * цел 1:2 иска от пазара повече, отколкото дава.
 *
 * В същия отрязък Инерция и Постоянен взеха SUI от 0.7264 до 0.7419 - едно
 * движение, +1.93%. Спринтьорът направи три сделки върху него: +0.54, -0.65,
 * -0.60. Нето минус. Честотата не добавя, а разпилява.
 *
